...
from crawler.strategies.xpath import XpathStrategy, _extract_deadlines_xpath
...
# --- XpathStrategy.extract (integration) ---

def test_strategy_registered():
    from crawler.strategy import _registry, _ensure_strategies_loaded
    _ensure_strategies_loaded()
    assert "xpath" in _registry


@patch("crawler.strategies.xpath._fetch")
def test_strategy_extract_basic(mock_fetch):
...
@patch("crawler.strategies.xpath._fetch")
def test_strategy_scaffolding_raises(mock_fetch):
