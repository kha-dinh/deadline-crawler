...
- V14: Deadline dates SHOULD be chronologically ordered per canonical sequence: abstract ≤ submission ≤ early_reject ≤ rebuttal_start ≤ rebuttal_end ≤ notification ≤ shepherd ≤ camera_ready. Violation emits warning (not error) — some conferences have unusual timelines. Only labels present in entry are checked. Known false positives (data correct, do not attempt to fix): (1) POPL: notification > shepherd — early shepherd contact before final notification; (2) UIST: notification > shepherd — conditional accept/shepherding before final decision; (3) EC: rebuttal_end > notification — two-round review, first-round notification before second-round rebuttal; (4) PODS Cycle 2: submission > notification — website typo (says Feb 2025, should be 2026)
