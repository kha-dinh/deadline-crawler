...
- I.crawl: Scrapy spider (`deadline_crawler/spiders/conferences.py`) — `ConferencesSpider` reads I.conf, generates requests per conference×year, dispatches to extractors in `crawler/extractors/`. Two-callback chain: `parse_cfp()` → optional `parse_with_main()` for `url_main`. Pipelines: `ValidationPipeline` (V1–V3, V10, V14, V16, V17, V19, V20, V21 checks) → `OutputPipeline` (collects items, writes JSON/YAML). `FixtureDownloaderMiddleware` serves local HTML when `FIXTURES_DIR` set. Compat layer (`crawler/compat.py`) provides `crawl_conference()` for unit tests without Scrapy reactor
...
| T30 | — | (planned) Continuous monitoring — HTTP cache (RFC2616Policy), `DiffPipeline` for change detection (`state.json`), `NotifyExtension` (console/JSONL/webhook), `--diff` flag, `monitor` subcommand, cron scheduling | I.crawl |
