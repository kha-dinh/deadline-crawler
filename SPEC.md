...
- V7: Every conference in conferences.yaml MUST have: name, strategy, area. `url` required unless `by_year` provides URLs for all target years
...
| T27 | x | CORE ranking from CSV — download `data/core2026.csv` from CORE portal (ICORE2026); `crawler/ranks.py` loads as `{acronym_lower: rank}`; `config.load_conferences` injects `rank` field; `core_acronym` overrides lookup; `core_rank` fallback for non-standard entries (NSDI); "unknown" when no rank found | I.conf,V3 |
